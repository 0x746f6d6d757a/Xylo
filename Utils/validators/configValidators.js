...
export function validateLoggerConfig(config) {
    ...
    if (typeof config.enabled !== 'boolean') {
        console.log('Validation failed: enabled is not a boolean')
        return false
    }
    if (typeof config.loggingLevel !== 'number' || ![1, 2, 3].includes(config.loggingLevel)) {
        console.log('Validation failed: loggingLevel is invalid')
        return false
    }
    
    if (!config.channels || typeof config.channels !== 'object' || config.channels === null) {
        console.log('Validation failed: channels is missing or not an object')
        return false
    }
    ...
    for (const [category, categoryData] of Object.entries(config.channels)) {
        if (!validCategories.includes(category)) {
            console.log(`Validation warning: unknown category ${category}, skipping`)
            continue
        }
        if (typeof categoryData !== 'object' || categoryData === null) {
            console.log(`Validation failed: categoryData for ${category} is not an object`)
            return false
        }
        if (typeof categoryData.enabled !== 'boolean') {
            console.log(`Validation failed: enabled for ${category} is not a boolean`)
            return false
        }
        
        if (categoryData.channelId !== null && categoryData.channelId !== undefined && typeof categoryData.channelId !== 'string') {
            console.log(`Validation failed: channelId for ${category} is invalid (must be null or string)`)
            return false
        }
        
        if (!categoryData.events || typeof categoryData.events !== 'object' || categoryData.events === null) {
            console.log(`Validation failed: events for ${category} is missing or not an object`)
            return false
        }
        
        for (const [eventName, eventData] of Object.entries(categoryData.events)) {
            if (typeof eventData !== 'object' || eventData === null) {
                console.log(`Validation failed: eventData for ${category}.${eventName} is not an object`)
                return false
            }
            if (typeof eventData.enabled !== 'boolean') {
                console.log(`Validation failed: enabled for ${category}.${eventName} is not a boolean`)
                return false
            }
        }
    }
